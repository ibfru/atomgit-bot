...
func TestRepositories_CRUD(t *testing.T) {
	...
	repo.Description = atomgit.String("description")
...
func TestRepositories_EditBranches(t *testing.T) {
	...
	protectionRequest := &atomgit.ProtectionRequest{
		RequiredStatusChecks: &atomgit.RequiredStatusChecks{
			Strict:   true,
			Contexts: &[]string{"continuous-integration"},
		},
		RequiredPullRequestReviews: &atomgit.PullRequestReviewsEnforcementRequest{
		...
		Restrictions:     nil,
		BlockCreations:   atomgit.Bool(false),
		LockBranch:       atomgit.Bool(false),
		AllowForkSyncing: atomgit.Bool(false),
	}
	...
	want := &atomgit.Protection{
		RequiredStatusChecks: &atomgit.RequiredStatusChecks{
			Strict:   true,
			Contexts: &[]string{"continuous-integration"},
		},
		RequiredPullRequestReviews: &atomgit.PullRequestReviewsEnforcement{
			DismissStaleReviews:          true,
			RequiredApprovingReviewCount: 0,
		},
		EnforceAdmins: &atomgit.AdminEnforcement{
			URL:     atomgit.String("https://api.atomgit.com/repos/" + *repo.Owner.Login + "/" + *repo.Name + "/branches/master/protection/enforce_admins"),
			Enabled: true,
		},
		Restrictions: nil,
		BlockCreations: &atomgit.BlockCreations{
			Enabled: atomgit.Bool(false),
		},
		LockBranch: &atomgit.LockBranch{
			Enabled: atomgit.Bool(false),
		},
		AllowForkSyncing: &atomgit.AllowForkSyncing{
			Enabled: atomgit.Bool(false),
		},
	}
...
func TestRepositories_ListByUser(t *testing.T) {
	...
	opt := atomgit.RepositoryListByUserOptions{Sort: "created"}
...
func TestRepositories_Autolinks(t *testing.T) {
	...
	opts := &atomgit.AutolinkOptions{
		KeyPrefix:      atomgit.String("TICKET-"),
		URLTemplate:    atomgit.String("https://example.com/TICKET?query=<num>"),
		IsAlphanumeric: atomgit.Bool(false),
	}
