...
const (
	...
	EventCustomToAccess = "__access__"
)
...
var (
	...
	eventTypeMapping = map[string]interface{}{
		...
		EventCustomToAccess:              &AccessEvent{},
...
func ParseWebHook(messageType string, payload []byte) (interface{}, error) {
	if strings.HasPrefix(messageType, EventCustomToAccess) {
		messageType = EventCustomToAccess
	}
	eventType, ok := messageToTypeName[messageType]
	if !ok {
		return nil, fmt.Errorf("unknown X-AtomGit-Event in message: %v", messageType)
	}
